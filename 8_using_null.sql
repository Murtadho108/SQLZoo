...
SELSECT teacher.name, dept.name FROM teacher RIGHT JOIN dept ON teacher.dept = dept.id;
...
SELECT teacher.name, COALESCE(dept.name,'None') Dept FROM teacher LEFT JOIN dept ON teacher.dept = dept.id;
